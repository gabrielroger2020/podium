...
export default function Teams (){
    ...
    let keywordView = "viewMyTeamsAthletes";
    ...
    const getTeams = (values)=>{
        let path = `/teams/select/athletes?user_id=${sessionStorage.getItem("id")}&`;
        ...
        api.get(path).then((response)=>{
            ...
            if(response.data.length > 0){
                ...
                response.data.forEach((team)=>{
                    let actions = <Flex direction={"row"} gap="1vh"><Button disabled={true} color="red" leftSection={<IconTrash></IconTrash>} onClick={()=>{
                        openDeleteModal(team.id);
                    }} >Excluir</Button><Button color="yellow" onClick={()=>{setSelectedTeam(team.id);modalEditHandlers.open()}} leftSection={<IconEdit></IconEdit>} disabled={true}>Editar</Button>
                    <Button onClick={()=>{
                        setSelectedTeam(team.id);
                        modalAthletesHandlers.open();
                    }}leftSection={<IconUsersGroup></IconUsersGroup>} color="blue">Atletas</Button></Flex>
    ...
    return(<>
        ...
        <Modal size="auto" opened={modalAthletes} onClose={()=>{
            ...
            <div className="baseMain">
                ...
                buttons={[<Button type="submit" color="blue" leftSection={<IconFilter></IconFilter>}>Filtrar</Button>]}>
