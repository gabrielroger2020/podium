...
export default function Managers(props){
    ...
    const getAthletes = ()=>{
        ...
        api.get(`/teams/athletes/${props.id}`).then((response)=>{
            let aux = [];
            if(response.data.length > 0){
                aux = response.data.map((element)=> {return `${element.users_id}`});
            }
            ...
            notifications.show({
                title: "Erro!",
                message: "Não foi possível selecionar os atletas dessa entidade.",
                color: "red"
            })
        })
    }
    ...
    const sendData = (values)=>{
        api.put(`/teams/athletes`, values).then((response)=>{
            notifications.show({
                title: "Sucesso!",
