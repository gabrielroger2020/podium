...
export default function View(props){
        ...
        api.get(`/teams/athletes/${props.id}`).then((response)=>{
            ...
            if(response.data.length > 0){
                response.data.forEach((athlete)=>{
                    ...
                    <Menu.Dropdown>
                    <Menu.Item onClick={()=>{openModalChangeStatusActive(athlete.id)}} leftSection={<IconPlayerPlay></IconPlayerPlay>}>
                            Ativar
                        </Menu.Item>
                        <Menu.Item onClick={()=>{openModalChangeStatusDisabled(athlete.id)}} leftSection={<IconPlayerPause></IconPlayerPause>}>
                            Desativar
                        </Menu.Item>
                    </Menu.Dropdown>
                </Menu>;
                    let actions = <Flex gap="1vh"><Button onClick={()=>{
                        setSelectedAthlete(athlete.users_id);
                        idCardHandlers.open();
                    }} leftSection={<IconEye></IconEye>}>Carteira de Ident.</Button>{alter_status}</Flex>;
    ...
    <Modal size="auto" title="Carteira de identificação" opened={idCard} onClose={()=>{idCardHandlers.close()}}>
        ...
        <Filters
            ...
            buttons={[<Button onClick={()=>{modalAthletesHandlers.open()}} color="blue" leftSection={<IconEdit></IconEdit>}>Gerenciar</Button>]}
        >
        </Filters>
