...
import { IconAlt, IconEdit, IconBuildingFactory2, IconBuildingSkyscraper, IconCalendar, IconCategory, IconHistory, IconHome, IconMail, IconMap, IconSchool, IconSignature, IconUsersGroup, IconWorldWww } from "@tabler/icons-react";
...
export default function Edit(props){
    ...
    const form = useForm({
        initialValues: {
            ...
            cnpj: (value) => ((new RegExp("^([0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}-[0-9]{2})|([0-9]{14})$").test(value) != true) && value.length < 18 && value != "" && value != null ? "O CNPJ deve ser preenchido corretamente." : null),
    ...
    const sendData = (values)=>{
        api.put("/entities/update", values).then((response)=>{
            notifications.show({
                title: "Sucesso!",
    ...
    const getInfosEntity = ()=>{
        api.get(`/entities/select?id=${props.id}`).then((response)=>{
            let aux = response.data[0];
            console.log(aux)
            aux.date_foundation = dayjs(aux.date_foundation);
            aux.category = `${aux.category}`;
            aux.address_number = `${aux.address_number}`;
            aux.cnpj = `${aux.cnpj}`;
            aux.inep_code = `${aux.inep_code}`;
            form.setValues(aux)
        }).catch((error)=>{
            console.log(error);
            notifications.show({
